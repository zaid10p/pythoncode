...
class BookParser:
    ...
    def rating(self):
        ...
        rating = BookParser.RATINGS.get(next(rating_classes), 'None')
        logger.info(f'Found book rating, `{rating}`.')
        return rating

    @property
    def imgUrl(self):
        logger.debug('Finding book img link...')
        locator = BookLocators.IMG_LOCATOR
        img_url = self.parent.select_one(locator).attrs['src']
        logger.info(f'Found book img link, `{img_url}`.')
        return img_url.replace("..", "http://books.toscrape.com")
